...
export class LiquifyEngine {
  ...
  restoreRegion(cx: number, cy: number, radius: number, feather = 0.18) {
    const safeRadius = clamp(radius, 0.001, 0.5);
    const featherScale = clamp(feather, 0, 0.45);
    const outerRadius = safeRadius * (1 + featherScale);
    const outerRadiusSq = outerRadius * outerRadius;
    const innerRadiusSq = safeRadius * safeRadius;
    const cols = this.cols;
    const rows = this.rows;
    const centerX = cx * (cols - 1);
    const centerY = cy * (rows - 1);
    const radiusX = outerRadius * (cols - 1);
    const radiusY = outerRadius * (rows - 1);
    ...
    for (let gy = rowStart; gy <= rowEnd; gy += 1) {
      ...
      for (let gx = colStart; gx <= colEnd; gx += 1) {
        ...
        if (distSq <= innerRadiusSq || outerRadius <= safeRadius) {
          this.deformX[idx] = 0;
          this.deformY[idx] = 0;
          continue;
        }
        ...
        const featherT = clamp((dist - safeRadius) / Math.max(outerRadius - safeRadius, 1e-6), 0, 1);
        const keep = smoothstep(0, 1, featherT);
        this.deformX[idx] *= keep;
        this.deformY[idx] *= keep;
      }
    }
  }
